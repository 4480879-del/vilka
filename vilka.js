...
class LiveBoardService {
    ...
    renderStartRow(r, amIRegistered = false, squadCounts = {}) { 
        ...
        // 1. Выносим расчет лимитов наружу
        let sMax = parseInt(r.squad_max);
        if (isNaN(sMax) || sMax < 1) sMax = 1;
        
        // 2. Жестко проверяем спортивный формат гонки
        const isTeamRace = r.race_format === 'ttt' || r.race_format === 'relay';
        if (isTeamRace && sMax <= 1) sMax = 4;

        // 3. Теперь плашка создается ТОЛЬКО если это командный формат И есть имя экипажа
        if (isTeamRace && r.squad_name) {
            let joinBtn = '';
            const currentSqCount = squadCounts[r.squad_id] || 1;
            
            if (currentSqCount < sMax && !amIRegistered && r.squad_id) {
                const crm = this.crm || (window.app && window.app.crm);
                const escaper = crm && crm.app ? crm.app.escapeHTML.bind(crm.app) : (window.app ? window.app.escapeHTML.bind(window.app) : (s)=>s);
                
                const sNameSafe = escaper(r.squad_name || '');
                const distIdSafe = r.distance_id || '';
                joinBtn = `<button onclick="window.app.joinSquadDirectly('${r.race_id}', '${r.squad_id}', '${sNameSafe}', '${distIdSafe}')" style="background:var(--success); color:#fff; border:none; padding:2px 6px; border-radius:4px; font-size:8px; font-family:'Unbounded'; font-weight:800; cursor:pointer; margin-left:6px; box-shadow: 0 2px 5px rgba(0,230,118,0.3); transition:0.2s;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='none'">🤝 ВСТУПИТЬ</button>`;
            }

            const crm = this.crm || (window.app && window.app.crm);
            const escaper = crm && crm.app ? crm.app.escapeHTML.bind(crm.app) : (window.app ? window.app.escapeHTML.bind(window.app) : (s)=>s);

            squadHtml = `<div style="margin-top: 4px;"><span style="background:rgba(168,85,247,0.15); color:#a855f7; padding:2px 4px; border-radius:4px; font-size:9px; font-family:'Unbounded'; font-weight:800; border:1px solid rgba(168,85,247,0.3); display:inline-flex; align-items:center;">ЭКИПАЖ: ${escaper(r.squad_name).toUpperCase()} (${currentSqCount}/${sMax})${joinBtn}</span></div>`;
        }
    ...
    async openLiveBoard(raceId, event) { 
        ...
        try {
            ...
            for (let raceData of racesData) {
                ...
                // 🔥 Сбор полов для командного деления
            const squadGendersMap = {};
            rosters.forEach(rosterRow => {
                if (rosterRow.squad_id) {
                    if (!squadGendersMap[rosterRow.squad_id]) squadGendersMap[rosterRow.squad_id] = [];
                    const gStr = String(rosterRow.expand?.rider_id?.gender || 'M').toUpperCase().trim();
                    squadGendersMap[rosterRow.squad_id].push(gStr);
                }
            });
                const rStartList = rosters.map(r => {
                    ...
                    try {
                        ...
                        if (crm && typeof crm.getRaceCategory === 'function') {
                            displayCat = crm.getRaceCategory({ ...rider, squad_id: r.squad_id, squad_genders: squadGendersMap[r.squad_id] || [] }, raceData);
                        }
                    ...
                    return {		
                        ...
                        race_id: r.race_id || raceData.id,             
                        squad_max: raceData.squad_max || 1,            
                        race_format: raceData.format || 'mass' 
                    };
                }).filter(Boolean);
            ...
            // 🔥 УМНАЯ ГРУППИРОВКА И СОРТИРОВКА ПАР В СТАРТ-ЛИСТЕ
this.currentLiveStartList.sort((a, b) => {
    // 1. Сначала проверяем время запланированного старта
    const startA = a.plannedStart || '';
    const startB = b.plannedStart || '';
    if (startA !== startB) return startA.localeCompare(startB);
    
    // Экипажи применяются строго для командных гонок (TTT) и эстафет
    const isTeamRace = a.race_format === 'ttt' || a.race_format === 'relay';

    // 2. Группируем по НАЗВАНИЮ экипажа только в командных гонках и эстафетах
    if (isTeamRace) {
        const nameA = a.squad_name || '';
        const nameB = b.squad_name || '';
        if (nameA !== nameB) {
            if (!nameA) return 1;  // Одиночки без экипажа летят в подвал минуты
            if (!nameB) return -1;
            return nameA.localeCompare(nameB);
        }
        
        // 3. Защита: разделяем разные экипажи с одинаковым названием
        const idA = a.squad_id || '';
        const idB = b.squad_id || '';
        if (idA !== idB) return idA.localeCompare(idB);
    }
    
    // 4. Сортируем напарников (или одиночек) по алфавиту (ФИО)
    ...
    renderLiveBoard() {
        ...
        const appRef = this.app || window.app;
        const amIRegistered = this.currentLiveStartList.some(r => r.id === (appRef && appRef.currentRider ? appRef.currentRider.id : null));
        const squadCounts = {};
        this.currentLiveStartList.forEach(r => {
        ...
        if (this.liveCurrentTab === 'result') {
            ...
            if (board.length === 0) html += `<tr><td colspan="4" style="text-align:center; padding:40px; color:var(--text-muted);">Старт-лист пуст</td></tr>`;
                ...
                if (isAbs) {
                    board.forEach(r => html += this.renderStartRow(r, amIRegistered, squadCounts));
                } else {
                    ...
                    groups.forEach(grp => {
                        ...
                        grpItems.forEach(r => html += this.renderStartRow(r, amIRegistered, squadCounts));
                    });
                }
            }
        }
...
// ==========================================
    ...
    async openRaceRoster(raceId, raceName, raceType) {
        ...
        try {
            ...
            if (freshRaceObj && freshRaceObj.category_order) {
                let parsedOrder = typeof freshRaceObj.category_order === 'string' ? JSON.parse(freshRaceObj.category_order) : freshRaceObj.category_order;
                this.localCategoryOrder = Array.isArray(parsedOrder) ? {} : (parsedOrder || {});
            }
        } catch(e) { console.error("Ошибка скачивания индексов категорий", e); }
        ...
        try {
            ...
            if (isOrganizer) {
                rosters.forEach(r => {
                    ...
                    let tName = this.getRiderTeamNames(rider); 
                ...
                } catch(e) { racePelotonId = currentRace ? currentRace.peloton_id : null; }
                ...
                Object.values(this.app.ridersMap).forEach(rider => {
                    ...
                    let hasMatchingPeloton = rTeams.length === 0; 
                    for (let tId of rTeams) {
                        ...
                        if (tObj) {
                            ...
                            if (!racePelotonId || teamPelotons.includes(racePelotonId)) { hasMatchingPeloton = true; break; }
                        }
                    }
                    ...
                    if (!combinedList.find(x => x.id === rider.id)) {
                        ...
                        let tName = this.getRiderTeamNames(rider); 
                        combinedList.push({ id: rider.id, name: rider.first_name, surname: rider.last_name, year: rider.yob, gender: gender, group: catCode, points: rider.rating || 0, team: tName, teamId: rider.team_id, isRegistered: false, recordId: null, plannedStart: null, bib: "", squad_name: "", squad_id: null, distance_id: null });
                    }
                });
                ...
                let teamRiders = [];
                if (activeTeamId) { teamRiders = await pb.collection('riders').getFullList({ filter: `team_id ~ "${activeTeamId}"`, sort: '-rating', requestKey: null }); }
                ...
                let tName = this.getRiderTeamNames(mp); 
                if (reg && reg.expand?.gruppetto_id) tName = `${tName} <span style="color:#a855f7; font-weight:800; font-size:10px;">[G: ${reg.expand.gruppetto_id.name}]</span>`;
                combinedList = [{ id: mp.id, name: mp.first_name, surname: mp.last_name, year: mp.yob, gender: gender, group: catCode, points: mp.rating || 0, team: tName, teamId: mp.team_id, isRegistered: !!reg, recordId: reg ? reg.id : null, plannedStart: reg ? reg.planned_start : null, bib: reg ? reg.bib : "", is_paid: reg ? reg.is_paid : false, squad_name: reg ? (reg.squad_name || "") : "", squad_id: reg ? (reg.squad_id || null) : null, distance_id: reg ? (reg.distance_id || null) : null }];
    ...
    const isTeamRace = raceObj && (raceObj.format === 'ttt' || raceObj.format === 'relay'); // 🔥 Гарантия соло-логики в групповых гонках

    // 🔥 ШАГ 1: ПРЕДРАСЧЕТ ПОЛОВ ДЛЯ ВСЕХ ЭКИПАЖЕЙ (Вставляем на первой строчке)
    const crmSquadGenders = {};
    combinedList.forEach(item => {
        if (item.isRegistered && item.squad_id) {
            if (!crmSquadGenders[item.squad_id]) crmSquadGenders[item.squad_id] = [];
            crmSquadGenders[item.squad_id].push(String(item.gender || 'M').toUpperCase().trim());
        }
    });
    ...
    // 🌟 Дубликат строки удален, теперь всё стабильно!
    
    // 🔥 ШАГ 2: ОБНОВЛЕННЫЙ ЦИКЛ РАСЧЕТА С ПЕРЕДАЧЕЙ Окружения
    filteredList.forEach(p => {
        p.computedCat = p.group || 'B'; 
        if (raceObj && typeof this.getRaceCategory === 'function') {
            try { 
                p.computedCat = this.getRaceCategory({ 
                    gender: p.gender, 
                    yob: p.year, 
                    base_cluster: p.group,
                    id: p.id,
                    squad_id: p.squad_id,
                    squad_genders: crmSquadGenders[p.squad_id] || [] // 🌟 Передаем готовый массив полов экипажа
                }, raceObj); 
            } catch (e) {}
        }
    });
    ...
    // 🔥 ПОДСЧЕТ МЕСТ В ЭКИПАЖАХ ДЛЯ "ОТКРЫТОГО ЛОББИ"
    const squadCounts = {};
    registeredRiders.forEach(p => {
        if (p.squad_id) squadCounts[p.squad_id] = (squadCounts[p.squad_id] || 0) + 1;
    });
    ...
    groupsArray.sort((a, b) => {
        ...
        return window.app.crm.getCatRank(catA) - window.app.crm.getCatRank(catB); // Отсортировано корректно
    });

    // 🔥 УМНАЯ КОМАНДНАЯ СОРТИРОВКА ВНУТРИ ГРУПП АДМИНКИ
    groupsArray.forEach(group => {
        group.riders.sort((a, b) => {
            ...
            // 🔥 Сортируем по экипажам только в TTT и эстафетах
            if (isTeamRace) {
                let squadA = a.squad_id || '';
                let squadB = b.squad_id || '';
                if (squadA !== squadB) {
                    if (!squadA) return 1;  
                    if (!squadB) return -1;
    ...
    if (filteredList.length === 0) { 
        ...
        const generateRowHtml = (p) => {
            ...
            let squadBadge = '';
            if (isTeamRace && p.squad_id) {
                const currentSqCount = squadCounts[p.squad_id] || 1;
                const sMax = raceObj ? (raceObj.squad_max > 1 ? raceObj.squad_max : ((raceObj.format==='ttt'||raceObj.format==='relay') ? 4 : 1)) : 1;
                
                let joinBtn = '';
                const amIRegistered = filteredList.some(r => r.id === (this.app.currentRider ? this.app.currentRider.id : null) && r.isRegistered);
                
                if (currentSqCount < sMax && !amIRegistered) {
                    joinBtn = `<button onclick="window.app.joinSquadDirectly('${raceObj.id}', '${p.squad_id}', '${this.app.escapeHTML(p.squad_name || '')}', '${p.distance_id || ''}')" style="background:var(--success); color:#fff; border:none; padding:2px 6px; border-radius:4px; font-size:8px; font-family:'Unbounded'; font-weight:800; cursor:pointer; margin-left:6px; box-shadow: 0 2px 5px rgba(0,230,118,0.3); transition:0.2s;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='none'">🤝 ВСТУПИТЬ</button>`;
                }
                
                squadBadge = `<br><span style="background:rgba(168,85,247,0.15); color:#a855f7; padding:2px 4px; border-radius:4px; font-size:9px; font-family:'Unbounded'; font-weight:800; border:1px solid rgba(168,85,247,0.3); margin-top: 4px; display: inline-flex; align-items: center;">ЭКИПАЖ: ${(p.squad_name || '').toUpperCase()} (${currentSqCount}/${sMax})${joinBtn}</span>`;
            }

            return `<tr style="${bg}"><td>${act}</td><td>${bibHtml}</td> <td><div style="display:flex; align-items:center; flex-wrap:nowrap;"><b style="font-size:13px; cursor:pointer; transition:0.2s; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" onmouseover="this.style.color='var(--primary)'" onmouseout="this.style.color=''" onclick="window.app.openProfile('${p.id}')" title="Открыть профиль">${p.name} ${p.surname}</b> ${paidBadge}</div>${squadBadge}</td><td style="max-width: 140px; white-space: normal; line-height: 1.3; word-wrap: break-word;"><span style="color:var(--text-muted); font-size:11px;">${p.team}</span></td><td><span style="color:var(--text-muted); font-family:'Roboto Mono'; font-size:12px;">${p.year}</span></td><td><div style="display:flex; align-items:center;">${genderBadge}${catHtml}</div></td><td>${timeHtml}</td><td><b style="color:var(--primary); font-size:13px; font-family:'Roboto Mono';">${p.points}</b></td></tr>`;
        };
        ...
        groupsArray.forEach(group => {
            ...
            if (isOrganizer) {
                ...
                if (isMergedGroup) {
                    unmergeBtnHtml = `<button onclick="window.app.crm.unmergeCategories('${raceObj.id}', '${catsJsonForUpdate}')" style="background:rgba(255,51,102,0.1); border:1px solid rgba(255,51,102,0.3); color:var(--danger); padding:4px 8px; border-radius:4px; font-size:9px; font-family:'Unbounded'; font-weight:800; cursor:pointer; margin-right: 10px;" title="Разделить на отдельные категории">✂️ РАЗДЕЛИТЬ</button>`;
                }
    ...
    let targetRider = this.raceRosterList.find(r => r.recordId === recordId);
    if (!targetRider) return;

    // Находим гонку и изолируем экипажи
    const currentRace = this.dataCalendar.find(r => r.id === this.openedEventId);
    const isTeamRace = currentRace && (currentRace.format === 'ttt' || currentRace.format === 'relay');
    const squadId = isTeamRace ? targetRider.squad_id : null;
    const cleanBib = newBib.trim();

    try {
        // 🔥 ЕСЛИ ЭТО КОМАНДНАЯ ГОНКА / ЭКИПАЖ (TTT или Пары)
        if (squadId) {
                // Находим всех напарников по экипажу в текущем списке
                const squadMembers = this.raceRosterList.filter(r => r.squad_id === squadId);
                
                // Синхронно обновляем всю банду в базе данных PocketBase
                await Promise.all(squadMembers.map(member => 
                    pb.collection('race_rosters').update(member.recordId, { bib: cleanBib }, { requestKey: null })
                ));

                // Обновляем данные в памяти и точечно меняем значения в инпутах на экране
                squadMembers.forEach(member => {
                    member.bib = cleanBib;
                    
                    // Бронебойный поиск инпута по уникальному ID записи в атрибуте onchange
                    const el = document.querySelector(`input[onchange*="${member.recordId}"]`);
                    if (el) {
                        el.value = cleanBib;
            ...
            // 🔥 ЕСЛИ ЭТО ОБЫЧНЫЙ ОДИНОЧНЫЙ ГОНЩИК
            else {
                await pb.collection('race_rosters').update(recordId, { bib: cleanBib }, { requestKey: null });
                targetRider.bib = cleanBib;
                setTimeout(() => { inputEl.style.backgroundColor = ''; }, 1000);
            }
        } catch(e) { 
            console.error("Ошибка сохранения BIB-номера:", e);
...
async updateCategory(recordId, newCat, inputEl) {
    ...
    async registerSingleRider(riderId, btn, event) {
        ...
        // 🔥 ФИКС: Сначала находим гонку в кэше календаря по её ID
const currentRace = this.dataCalendar.find(r => r.id === this.openedEventId);
...
if (currentRace && currentRace.level === 'peloton') {
   ...
   async generateBibs() {
        ...
        if (!confirm("Автоматически присвоить номера ВСЕМ заявленным гонщикам строго по ВРЕМЕНИ СТАРТА с объединением экипажей TTT?")) return;

        // 🔥 ПРЕДРАСЧЕТ ПОЛОВ ДЛЯ АВТОГЕНЕРАТОРА НОМЕРОВ
        const crmSquadGenders = {};
        sortedRiders.forEach(item => {
            if (item.squad_id) {
                if (!crmSquadGenders[item.squad_id]) crmSquadGenders[item.squad_id] = [];
                crmSquadGenders[item.squad_id].push(String(item.gender || 'M').toUpperCase().trim());
            }
        ...
        // 🔥 ШАГ 1: Подготавливаем категории
        const raceObj = this.dataCalendar.find(r => r.id === this.openedEventId);
		const isTeamRace = raceObj && (raceObj.format === 'ttt' || raceObj.format === 'relay'); // 🔥 Защита автогенератора
            ...
            if (raceObj && typeof this.getRaceCategory === 'function') {
                try { 
                    p.computedCat = this.getRaceCategory({ 
                        ...
                        id: p.id,
                        squad_id: p.squad_id,
                        squad_genders: crmSquadGenders[p.squad_id] || []
                    }, raceObj); 
                } catch(e) {}
            }
        ...
        // 🔥 ШАГ 2: ГЛОБАЛЬНАЯ СОРТИРОВКА (Интегрирован замок для TTT)
        sortedRiders.sort((a, b) => {
            ...
            // ⭐ ДОБАВЛЕНО: Группируем напарников по экипажу вместе внутри одной минуты старта,
            // чтобы их не раскидало по очкам или алфавиту дальше.
            if (isTeamRace) {
    let squadA = a.squad_id || '';
    let squadB = b.squad_id || '';
    if (squadA !== squadB) {
        ...
        return squadA.localeCompare(squadB);
            ...
            // 2. Если время старта и экипаж совпадают (или оба соло) - смотрим на приоритет группы
        ...
        // 🔥 ШАГ 3: Раздаем номера (Умный инкремент для TTT)
        try {
            const squadBibMap = {}; // Карта выданных номеров для экипажей: { squad_id: "номер" }
            let bibCounter = 1;
            ...
            for (let i = 0; i < sortedRiders.length; i++) {
                ...
                // Если гонщик едет в экипаже
                if (isTeamRace && rider.squad_id) {
                    // Если этому экипажу мы ЕЩЕ НЕ выдавали номер в этом цикле — генерируем новый
                    if (!squadBibMap[rider.squad_id]) {
                        squadBibMap[rider.squad_id] = bibCounter.toString();
                        bibCounter++; // Увеличиваем счетчик только для следующего экипажа
                    }
                    // Присваиваем гоншку общий номер его команды
                    assignedBib = squadBibMap[rider.squad_id];
                } 
                // Если едет одиночка
                else {
                    assignedBib = bibCounter.toString();
                    bibCounter++;
                }

                // Отправляем в PocketBase
                await pb.collection('race_rosters').update(rider.recordId, { bib: assignedBib }, { requestKey: null });
            }

            alert("✅ Стартовые номера успешно присвоены! Экипажи TTT получили одинаковые BIB.");
            
            // 🔥 ШАГ 4: Перерисовываем интерфейс оригинальным методом
            this.openRaceRoster(this.openedEventId, this.openedEventName, this.openedEventType);
            
        } catch(e) { 
            console.error("Ошибка автоматического присвоения:", e);
...
async generateITTStarts() {
        ...
        // 🔥 ПРЕДРАСЧЕТ ПОЛОВ ДЛЯ ITT ГЕНЕРАТОРА
        const crmSquadGenders = {};
        sortedRiders.forEach(item => {
            if (item.squad_id) {
                if (!crmSquadGenders[item.squad_id]) crmSquadGenders[item.squad_id] = [];
  ...
  async openWaveStartModal() {
        ...
        try {
            ...
            // 🔥 ПРЕДРАСЧЕТ ПОЛОВ ДЛЯ МОДАЛКИ ВОЛН
            const crmSquadGenders = {};
            riders.forEach(item => {
                if (item.squad_id) {
                    if (!crmSquadGenders[item.squad_id]) crmSquadGenders[item.squad_id] = [];
            ...
            riders.forEach(r => {
                ...
                if (!r.tempCat) {
                    ...
                    if (raceObj && catCalculator) {
                        try { 
                            r.tempCat = catCalculator({ 
                                gender: r.gender, 
                                yob: r.year, 
                                base_cluster: r.group,
                                id: r.id,
                                squad_id: r.squad_id,
                                squad_genders: crmSquadGenders[r.squad_id] || []
                            }, raceObj); 
                        } catch (e) {}
                    }
                }
    ...
    async applyWaveStarts() {
        ...
        // 🔥 ПРЕДРАСЧЕТ ПОЛОВ ДЛЯ ПРИМЕНЕНИЯ ВОЛН
        const crmSquadGenders = {};
        riders.forEach(item => {
            if (item.squad_id) {
                if (!crmSquadGenders[item.squad_id]) crmSquadGenders[item.squad_id] = [];
        ...
        try {
            for (let rider of riders) {
                ...
                if (!computedCat) {
                    ...
                    if (raceObj && catCalculator) {
                        try { 
                            computedCat = catCalculator({ 
                                gender: rider.gender, 
                                yob: rider.year, 
                                base_cluster: rider.group,
                                id: rider.id,
                                squad_id: rider.squad_id,
                                squad_genders: crmSquadGenders[rider.squad_id] || []
                            }, raceObj); 
                        } catch (e) {}
                    }
                }
...
// ==========================================
    ...
    getRaceCategory(rider, race) {
		...
		if (logic === 'Командные' || logic === 'teams') logic = 'teams';
		...
		// 🔥 УМНОЕ КОМАНДНОЕ ДЕЛЕНИЕ НАЛЕТУ
        // 🔥 НАШЕ НАДЁЖНОЕ КОМАНДНОЕ ДЕЛЕНИЕ
        // 🔥 НАДЁЖНОЕ КОМАНДНОЕ ДЕЛЕНИЕ
        if (logic === 'teams') {
            let genders = rider.squad_genders || [];
            
            // Предохранитель: если массив пустой (одиночный гонщик), берем его личный пол
            if (genders.length === 0) {
                genders.push(String(rider.gender || 'M').toUpperCase().trim());
            }
            
            const hasM = genders.some(g => g === 'M' || g === 'М');
            const hasF = genders.some(g => g === 'F' || g === 'Ж');
            
            if (hasM && hasF) return 'МИКСТ';
            if (hasF) return 'ЖЕНЩИНЫ';
            return 'МУЖЧИНЫ';
        }
    ...
    async processCsvData(csvText, raceId) {
        try {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                let generatedCategory = this.getRaceCategory(
    { gender: riderGender, yob: yob, base_cluster: newCluster },
    currentRace
);
...
// 🔥 УНИВЕРСАЛЬНЫЙ ПОЛУЧАТЕЛЬ ИМЕНИ КОМАНДЫ (МУЛЬТИ-КЛУБ)
        getRiderTeamName(rider) {
            ...
            if (this.currentPelotonFilter && this.currentPelotonFilter !== 'all') {
                const activeTid = riderTeams.find(id => {
    ...
    try {
        ...
        if (lbl) {
            lbl.innerHTML = `ВСЕ (ГЛОБАЛЬНО) <span style="font-size: 8px;">▼</span>`;
                ...
                setTimeout(async () => {
                    // 🔥 1. УМНЫЙ ПАРСИНГ: Защита от Телеграма (который меняет ? на # в ссылках)
                    const urlString = window.location.search || window.location.hash.replace('#', '?');
                    const safeParams = new URLSearchParams(urlString);

                    // Берем параметры или из умного парсера, или из старого
                    const tChat = safeParams.get('chat') || urlParams.get('chat');
                    const tUser = safeParams.get('user') || urlParams.get('user');
                    const tTeam = safeParams.get('team') || urlParams.get('team');
                    const tJoin = safeParams.get('join_team') || urlParams.get('join_team'); 
                    const tPeloton = safeParams.get('peloton') || urlParams.get('peloton'); 
                    const tCalendar = safeParams.get('calendar') || urlParams.get('calendar');
                    
                    const tJoinSquadId = safeParams.get('join_squad') || urlParams.get('join_squad');
                    const tSquadName = safeParams.get('sname') || urlParams.get('sname');
                    const tSquadRaceId = safeParams.get('squad_race') || urlParams.get('squad_race');
                    const tSquadDistId = safeParams.get('squad_dist') || urlParams.get('squad_dist');					

                    if (tJoinSquadId && tSquadName && tSquadRaceId) {
                        const decodedName = decodeURIComponent(tSquadName);
                        
                        // 🔥 2. ГЛАВНЫЙ ФИКС: Сохраняем в память СРАЗУ (до всяких проверок на гостя)!
                        // Теперь инвайт доживет до момента, когда друг успешно авторизуется.
                        sessionStorage.setItem('pending_squad_id', tJoinSquadId);
                        sessionStorage.setItem('pending_squad_name', decodedName);
                        ...
                        if (this.isGuest) {
                            if (confirm(`👋 Вас пригласили в экипаж «${decodedName}»!\n\nДля участия необходимо войти в аккаунт SOTKA. Перейти ко входу?`)) {
                                this.openLoginScreen();
                            }
                        } else {
                            alert(`👋 Вас пригласили в экипаж «${decodedName}»!\n\nСейчас откроется карточка гонки. Нажмите фиолетовую кнопку "ВСТУПИТЬ", чтобы занять свое место.`);
                        ...
                        const cleanUrl = tChat ? window.location.pathname + '?chat=' + tChat : window.location.pathname;
                        window.history.replaceState({}, document.title, cleanUrl);
                        return;
                    }

                    // 🔥 3. АВТО-ОТКРЫТИЕ ПОСЛЕ ВХОДА ДРУГА
                    // Если гость вошел в аккаунт, ссылка стерлась, но инвайт в памяти остался:
                    const savedSquadRace = sessionStorage.getItem('pending_squad_race');
                    if (savedSquadRace && !this.isGuest && !tChat && !tCalendar && !tTeam && !tUser) {
                        const rChat = this.chats.find(c => c.race_id === savedSquadRace);
                        if (rChat) {
                            this.chatListFilter = 'races';
                            this.openChat(rChat.id);
                        }
...
async registerForRace(raceId, btn, event, wave = null) {
        ...
        allBtns.forEach(b => { b.innerHTML = '⌛ ОБРАБОТКА...'; b.style.opacity = '0.7'; b.style.pointerEvents = 'none'; });
        ...
        try {
            ...
            // 🔥 Определяем лимит ОДИН РАЗ для всей функции
            let squadMax = parseInt(race.squad_max);
if (isNaN(squadMax) || squadMax < 1) squadMax = 1;
if ((race.format === 'ttt' || race.format === 'relay') && squadMax <= 1) squadMax = 4;
...
const isTeamRace = race.format === 'ttt' || race.format === 'relay';
            ...
            if (myRegistration) {
                // 🔥 ПЕРЕХВАТ КЛИКА: Если я уже в экипаже и он не полон — просто копируем ссылку
                if (myRegistration.squad_id && squadMax > 1) {
                    const currentSquadCount = existing.filter(r => r.squad_id === myRegistration.squad_id && r.status !== 'canceled').length;
                    if (currentSquadCount < squadMax) {
                        const inviteUrl = `${window.location.origin}/?chat=${this.activeChatId}&join_squad=${myRegistration.squad_id}&sname=${encodeURIComponent(myRegistration.squad_name)}&squad_race=${raceId}`;
                        this.copyText(inviteUrl, `✅ Экипаж «${myRegistration.squad_name}» ждет напарников!\n\nИнвайт-ссылка скопирована.\nВы можете отправить её в чат, или ваши друзья могут просто нажать «Вступить» прямо в старт-листе гонки!`);
                        allBtns.forEach(b => { b.innerHTML = originalHtml; b.style.opacity = '1'; b.style.pointerEvents = 'auto'; });
                        return;
                    }
                }
            ...
            if (squadMax > 1) {
                ...
                if (pendingSquadId && pendingSquadRace === raceId) {
                    ...
                    if (currentSquadRosters.length >= squadMax) {
                    ...
                    if (isAdminOrJudge || isCaptain) {
                        this.openSquadBuilderModal(raceId, squadMax, race.name, wave, null);
                        allBtns.forEach(b => { b.innerHTML = originalHtml; b.style.pointerEvents = 'auto'; b.style.opacity = '1'; });
                        return;
                    } else {
                        const squadNameInput = prompt(`Эта гонка командная (Экипаж до ${squadMax} чел).\n\nПридумайте название для вашего экипажа (Оно будет в протоколах):`);
                        ...
                        this.copyText(inviteUrl, `✅ Экипаж «${targetSquadName}» создан!\n\nИнвайт-ссылка скопирована в буфер обмена. Вы можете отправить её друзьям!\nИли они могут зайти в старт-лист гонки и просто нажать кнопку «Вступить».`);
                    }
                }
            }
...
const isTeamRace = race.format === 'ttt' || race.format === 'relay';
            ...
            if (myRegistration) {
                // 🔥 ПЕРЕХВАТ КЛИКА: Если я уже в экипаже и он не полон — просто копируем ссылку
                if (myRegistration.squad_id && squadMax > 1) {
                    const currentSquadCount = allRaceRosters.filter(r => r.squad_id === myRegistration.squad_id && r.status !== 'canceled').length;
                    if (currentSquadCount < squadMax) {
                        const inviteUrl = `${window.location.origin}/?chat=${this.activeChatId}&join_squad=${myRegistration.squad_id}&sname=${encodeURIComponent(myRegistration.squad_name)}&squad_race=${raceId}&squad_dist=${distanceId}`;
                        this.copyText(inviteUrl, `✅ Экипаж «${myRegistration.squad_name}» ждет напарников!\n\nИнвайт-ссылка скопирована.\nВы можете отправить её в чат, или ваши друзья могут просто нажать «Вступить» прямо в старт-листе гонки!`);
            ...
            if (squadMax > 1) {
                ...
                if (pendingRaceId === raceId && pendingSquadId && (!pendingSquadDist || pendingSquadDist === distanceId)) {
                    ...
                    if (isAdminOrJudge || isCaptain) {
                        ...
                        this.copyText(inviteUrl, `✅ Экипаж «${finalSquadName}» создан!\n\nСсылка-приглашение скопирована в буфер обмена. Вы можете отправить её друзьям!\nИли они могут зайти в старт-лист гонки и просто нажать кнопку «Вступить».`);
                    }
                }
            }
	...
	// 🔥 НОВОЕ: Мгновенное вступление в чужой экипаж из таблицы старт-листа
    async joinSquadDirectly(raceId, squadId, squadName, distanceId) {
        if (!this.currentRider) return;
        if (this.isGuest || (this.currentRider.email && this.currentRider.email.startsWith('guest_'))) {
            if (confirm("Для вступления в экипаж необходимо войти в аккаунт SOTKA. Перейти ко входу?")) {
                this.openLoginScreen();
            }
            return;
        }

        if (!confirm(`Вы уверены, что хотите присоединиться к экипажу «${squadName}»?`)) return;

        try {
            const [existing, race] = await Promise.all([
                pb.collection('race_rosters').getFullList({ filter: `squad_id="${squadId}" && status!="canceled"`, requestKey: null }),
                pb.collection('races').getOne(raceId, { requestKey: null })
            ]);
            
            let sMax = race.squad_max || 1;
            if ((race.format === 'ttt' || race.format === 'relay') && sMax <= 1) sMax = 4;

            if (existing.length >= sMax) {
                return alert(`⛔ Мест нет! Экипаж «${squadName}» уже полностью укомплектован.`);
            }
            ...
            if (distanceId && distanceId !== 'undefined' && distanceId !== 'null') {
                payload.distance_id = distanceId;
            }

            await pb.collection('race_rosters').create(payload, { requestKey: null });
            
            alert(`✅ Вы успешно присоединились к экипажу «${squadName}»!\nНажмите на желтую кнопку "Оплатить взнос", чтобы подтвердить участие.`);
            this.syncRaceButtonsState();
            
            // Если таблица старт-листа открыта - перерисовываем её мгновенно
            if (this.crm && typeof this.crm.loadData === 'function' && document.getElementById('raceRosterTableContainer')) {
                this.crm.loadData();
            }
        } catch (e) {
            console.error(e);
            alert("Ошибка при вступлении в экипаж.");
        }
    }
...
// ==========================================
  ...
  // ==========================================
    ...
    async openSquadBuilderModal(raceId, squadMax, raceName, wave, distanceId) {
        ...
        // 1. Берем команды текущего юзера (включая ONE TEAM, если он там)
        const myTeams = Array.isArray(this.currentRider.team_id) ? this.currentRider.team_id : (this.currentRider.team_id ? [this.currentRider.team_id] : []);
        
        // 2. Отбираем гонщиков: только тех, у кого есть ОБЩИЕ КОМАНДЫ со мной
        let availableRiders = Object.values(this.ridersMap).filter(r => {
            // Себя показываем всегда
            if (r.id === this.currentRider.id) return true;
            // Исключаем ботов и гостей
            if (r.email === 'bot@sotka.one' || (r.email && r.email.startsWith('guest_'))) return false;

            const rTeams = Array.isArray(r.team_id) ? r.team_id : (r.team_id ? [r.team_id] : []);
            
            // Проверяем пересечение: есть ли у нас хоть одна общая команда?
            return rTeams.some(tId => myTeams.includes(tId));
        });
        ...
        availableRiders.forEach(r => {
            ...
            // 🔥 ФИКС: Убрали автовыбор капитана. Теперь все чекбоксы изначально пустые
            const checked = ''; 
            const bg = isReg ? 'var(--bg-surface)' : 'transparent';
            
            // Строка для поиска (Имя Фамилия в нижнем регистре)
            const searchStr = this.escapeHTML((r.last_name + ' ' + r.first_name).toLowerCase());
            
            ridersHtml += `
                <label class="squad-rider-label" data-search="${searchStr}" style="display:flex; align-items:center; gap:10px; padding:10px; border-bottom:1px solid var(--border); background:${bg}; opacity:${op}; cursor:${isReg ? 'not-allowed' : 'pointer'}; transition:0.2s;" onmouseover="if(!${isReg}) this.style.background='var(--bg-surface-hover)'" onmouseout="if(!${isReg}) this.style.background='transparent'">
        ...
        modal.innerHTML = `
            <div class="modal-box" style="max-width:500px; width:95%; max-height:90vh; display:flex; flex-direction:column; padding:0; overflow:hidden;">
                ...
                <div style="padding:20px; overflow-y:auto; flex:1;">
                    ...
                    <input type="text" id="squadBuilderSearch" class="auth-input" style="width:100%; margin-top:5px; margin-bottom:10px;" placeholder="🔍 Поиск по команде..." oninput="window.app.filterSquadBuilderRiders(this.value)">

                    <div style="border:1px solid var(--border); border-radius:8px; max-height:300px; overflow-y:auto;">
                        ${ridersHtml || '<div style="padding:20px; text-align:center; color:var(--text-muted); font-size:12px;">В вашей команде больше нет доступных гонщиков</div>'}
                    </div>
                </div>
    ...
    // 🔥 НОВОЕ: Функция для работы строки поиска в модалке экипажа
    filterSquadBuilderRiders(query) {
        const q = query.toLowerCase().trim();
        const labels = document.querySelectorAll('.squad-rider-label');
        labels.forEach(lbl => {
            const name = lbl.getAttribute('data-search') || '';
            if (name.includes(q)) {
    ...
    async submitSquadBuilder(raceId, squadMax, wave, distanceId) {
        ...
        try {
            // 🔥 ФИКС ДУБЛЕЙ: Проверяем, не занято ли имя на этой гонке прямо сейчас
            const currentRosters = await pb.collection('race_rosters').getFullList({ 
                filter: `race_id="${raceId}" && status!="canceled"`, 
                requestKey: null 
            });
            const isNameTaken = currentRosters.some(r => r.squad_name && r.squad_name.toLowerCase().trim() === nameInput.toLowerCase().trim());
            if (isNameTaken) {
                alert(`❌ Название «${nameInput}» уже занято другим экипажем на этой гонке! Пожалуйста, придумайте уникальное имя.`);
